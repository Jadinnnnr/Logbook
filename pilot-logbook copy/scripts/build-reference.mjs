...
/**
 * Parts eCFR wouldn't serve this run.
 *
 * Tracked rather than merely warned about: a reference database missing whole
 * parts is far worse than one that failed outright, because it looks complete.
 * Searching it for something in a missing part returns nothing at all, which is
 * indistinguishable from the regulation not existing.
 */
const skippedParts = [];

/**
 * A machine-readable progress line for the orchestrator to parse.
 *
 * Printed alongside the human output rather than instead of it: the log stays
 * readable, and refresh-data.mjs picks these out to drive the progress bar.
 * Without them a bar could only move a third at a time, which over a ten-minute
 * rebuild tells you nothing about whether it's alive.
 */
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (!res.ok) {
        ...
        // eCFR returns 502/503 under load and recovers on its own. The old
        // three tries a second and a half apart gave up while it was still
        // catching its breath, and a part dropped that way vanished from the
      ...
      // Honour Retry-After where it's given; otherwise back off exponentially
      // to about half a minute rather than hammering a struggling server.
      const backoff = e.retryAfter ? e.retryAfter * 1000 : Math.min(30000, 2000 * 2 ** i);
...
for (const part of PARTS) {
  ...
  progress(PARTS.indexOf(part) + 1, PARTS.length + 1, `14 CFR part ${part}`);
}
...
try {
  ...
  for (const href of hrefs) {
    ...
    // The AIM counts as the final part-sized unit of work.
    progress(
      PARTS.length + (hrefs.indexOf(href) + 1) / hrefs.length,
      PARTS.length + 1,
      "AIM"
    );
  }
...
setMeta.run("skipped_parts", skippedParts.join(","));
setMeta.run("parts_expected", String(PARTS.length));
setMeta.run("parts_built", String(PARTS.length - skippedParts.length));
...
// The database is kept either way — fifteen parts beat none, and the pilot may
// well need one of them today. But the run does not get to call itself a
// success: a silent "done" over a database missing 14 CFR 67 would leave the
// medical standards unsearchable with a fresh timestamp next to it.
if (skippedParts.length > 0) {
  console.error(
    `\nIncomplete: 14 CFR ${skippedParts.join(", ")} could not be fetched and ` +
      `${skippedParts.length === 1 ? "is" : "are"} missing from the database. ` +
      `Everything else was written. Run this again when eCFR is answering.`
  );
  process.exit(1);
}
