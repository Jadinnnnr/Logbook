...
export function readChart(data: ChartData, entry: Entry): ChartReading {
  ...
  // Where each drawn curve sits at this temperature.
  const coordAt = (i: number) => {
    const c = alts[i].curve;
    const lo = c[0][0], hi = c[c.length - 1][0];
    if (entry.oatC < lo - 0.5 || entry.oatC > hi + 0.5) {
      const msg =
        `${entry.oatC} °C is outside the temperatures figure ${data.figure} covers at ` +
        `${alts[i].alt.toLocaleString()} ft (${Math.round(lo)} °C to ${Math.round(hi)} °C); ` +
