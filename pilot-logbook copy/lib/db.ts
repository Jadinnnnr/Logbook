...
import type { Profile as WBProfile } from "./weightbalance.ts";
import { BUILT_IN_PROFILES } from "./weightbalance.ts";
import fs from "fs";
...
-- Weight-and-balance profiles a pilot added. The built-in aircraft live in
-- code; only custom ones are stored, as JSON so a profile can gain a field
-- without a migration for every one of them.
CREATE TABLE IF NOT EXISTS told_profiles (
  id TEXT PRIMARY KEY,
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  json TEXT NOT NULL,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
...
/**
 * A pilot's own profiles, in the order they made them. Built-ins are not
 * stored: they're code, and a saved copy would silently shadow a corrected one
 * on the next deploy.
 */
export function toldProfilesForUser(userId: number): WBProfile[] {
  const rows = getDb()
    .prepare("SELECT json FROM told_profiles WHERE user_id = ? ORDER BY created_at, id")
    .all(userId) as { json: string }[];
  const out: WBProfile[] = [];
  for (const row of rows) {
    try {
      out.push(JSON.parse(row.json) as WBProfile);
    } catch {
      // A profile that won't parse is skipped rather than taking the page down.
    }
  }
  return out;
}

/** Built-ins first, then the pilot's own. */
export function allToldProfiles(userId: number): WBProfile[] {
  return [...BUILT_IN_PROFILES, ...toldProfilesForUser(userId)];
}
