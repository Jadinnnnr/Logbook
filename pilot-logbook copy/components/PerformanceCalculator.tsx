 ...
 *
 * The two halves used to sit together, and the distance card still leans on the
 * conditions above it — pressure altitude, temperature, and the headwind on the
 * chosen runway all fall back to what Conditions worked out. Splitting them
 * across two pages keeps one component so those fallbacks stay in one place;
 * on the TOLD page there is simply nothing above to fall back to, and the card
 * says so rather than pointing at a block that isn't there.
 */
export type PerformanceSection = "conditions" | "distance";
...
export default function PerformanceCalculator({
  ...
  /** Takeoff weight carried across from the weight-and-balance card. */
  ...
  const [grossWeight, setGrossWeight] = useState(
    defaultGrossWeight && defaultGrossWeight > 0 ? String(Math.round(defaultGrossWeight)) : ""
  );
  ...
  const pohResults = useMemo(() => {
    ...
    if (!Number.isFinite(pohPaValue)) {
      return {
        blocked:
          section === "conditions"
            ? "Enter a pressure altitude, or a field elevation and altimeter setting in Conditions."
            : "Enter a pressure altitude. The Performance page works it out from an airport's elevation and altimeter setting.",
      };
    }
    if (!Number.isFinite(pohOatValue)) {
      return {
        blocked:
          section === "conditions"
            ? "Enter a temperature, here or in Conditions."
            : "Enter a temperature.",
      };
    }
    if (!Number.isFinite(w)) return { blocked: "Enter a gross weight." };
    if (!Number.isFinite(pohHeadwind)) {
      return {
        blocked:
          section === "conditions"
            ? "Enter a headwind component, or pick an airport with runways."
            : "Enter a headwind component. The Performance page computes it from an airport's runways and the current METAR.",
      };
    }
    const percents = CORRECTIONS.map((c) => num(corrections[c.key])).filter(Number.isFinite);
    return {
  ...
  }, [usePoh, pohPaValue, pohOatValue, grossWeight, pohHeadwind, corrections, section]);
