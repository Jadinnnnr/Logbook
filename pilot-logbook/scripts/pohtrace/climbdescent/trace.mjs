...
/**
 * Trim a carpet curve back to the part that is really the curve.
 *
 * A pressure-altitude curve climbs (5-17) or falls (5-37) steadily across the
 * whole carpet and stops at the ISA envelope. Where the follower runs off the
 * end of one it lands on the envelope line or a major rule, and that shows up as
 * one of two things at an end: a step back the way it came, or a dead-flat run.
 * Neither can be the curve. Left in, figure 5-17's 6,000 ft line ran flat for a
 * further 13 °C past the envelope and several curves started with a 50–100 px
 * hook, both of which are well outside the width of the printed line.
 *
 * Capped at 15% per end so a misjudged direction cannot eat a whole curve.
 */
function reliableSpan(ys, { win = 25, tol = 3, flatRun = 90, flatBand = 1 } = {}) {
  const n = ys.length;
  const q = Math.floor(n / 4);
  const dir = Math.sign(ys[n - 1 - q] - ys[q]) || 1;
  const cap = Math.floor(n * 0.15);

  const flatAt = (i, step) => {
    let lo = ys[i], hi = ys[i];
    for (let k = 0; k < flatRun; k++) {
      const j = i + k * step;
      if (j < 0 || j >= n) return false;
      lo = Math.min(lo, ys[j]); hi = Math.max(hi, ys[j]);
    }
    return hi - lo <= flatBand;
  };

  // Measured across a window, not step to step: a hook slides 100 px the wrong
  // way over 60 columns, which is under 2 px a column and invisible one step at
  // a time. The flattest curve on either chart still moves the right way over a
  // window this size, so only a real reversal trips this.
  const reversedAt = (i, step) => {
    const j = i + step * win;
    if (j < 0 || j >= n) return false;
    return (ys[j] - ys[i]) * dir * step < -tol;
  };

  // Cut to the last bad column, not the first good one. A hook is not uniformly
  // bad along its length — one column in the middle of it can read clean, and
  // stopping there leaves most of the hook in place.
  let a = 0, b = n;
  for (let i = 0; i < cap; i++) if (reversedAt(i, 1) || flatAt(i, 1)) a = i + 1;
  for (let i = 0; i < cap; i++) {
    const j = n - 1 - i;
    if (reversedAt(j, -1) || flatAt(j, -1)) b = j;
  }
  return { a, b };
}
...
export function buildChart(pageNo) {
  ...
  const traced = all.filter(c => c.x1 - c.x0 >= spanCut).map(c => {
    const rawY = c.pts.map(p => p.y + Y0);
    // Trim before smoothing: a 21-wide average would otherwise drag the hook
    // back into the samples either side of it.
    const { a, b } = reliableSpan(rawY);
    if (a || b < rawY.length) {
      console.warn(`  ${spec.figure}: trimmed ${a} + ${rawY.length - b} columns off a carpet curve`);
    }
    return {
      oats: c.pts.slice(a, b).map(p => (p.x + spec.carpet.x0 - spec.oat.zero) / spec.oat.per + spec.oat.at),
      pys: smooth(rawY.slice(a, b)),
    };
  });
