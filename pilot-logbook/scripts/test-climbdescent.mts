...
// --- the traced curves themselves ---
//
// The checks above all read the middle of the chart, where the example lives.
// These read the ends, which is where a follower that has run off a curve onto
// an ISA line or a major rule shows up: as a step back the way it came, or as a
// dead-flat run. Both were present once and neither was visible from the
// example.
for (const chart of CHARTS) {
  for (const a of chart.data.altitudes) {
    const c = a.curve;
    const dir = Math.sign(c[c.length - 1][1] - c[0][1]);
    const worst = c.slice(1).reduce((m, p, i) => Math.min(m, (p[1] - c[i][1]) * dir), 0);
    must(`figure ${chart.figure}: the ${a.alt.toLocaleString()} ft curve never doubles back`,
      worst > -1.5, `worst step ${worst.toFixed(1)} px against the run of the curve`);

    const flat = c.slice(1).filter((p, i) => Math.abs(p[1] - c[i][1]) < 0.05).length;
    must(`figure ${chart.figure}: the ${a.alt.toLocaleString()} ft curve is nowhere dead flat`,
      flat <= 1, `${flat} of ${c.length - 1} steps go nowhere — a rule, not a curve`);
  }
}

// Reading a curve that the tracer only partly recovered must not silently fall
// back to the end of the stump. Figure 5-17's 7,000 ft line starts at +5 °C, so
// a cold day there is carried over from a neighbouring altitude instead.
const cold7 = readChart(CLIMB_CHART.data, { pressureAlt: 7000, oatC: 0 });
const cold6 = readChart(CLIMB_CHART.data, { pressureAlt: 6000, oatC: 0 });
const cold8 = readChart(CLIMB_CHART.data, { pressureAlt: 8000, oatC: 0 });
must("a partly traced curve is filled in from its neighbours, not clamped",
  cold7.timeMin > cold6.timeMin && cold7.timeMin < cold8.timeMin,
  `6,000 ${cold6.timeMin} / 7,000 ${cold7.timeMin} / 8,000 ${cold8.timeMin} min`);
must("...and does not cry off the chart for a temperature the chart plainly covers",
  cold7.warnings.length === 0, cold7.warnings.join(" | "));
